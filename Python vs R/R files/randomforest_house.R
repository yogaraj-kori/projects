...
rf_model <- randomForest(Appliances ~ ., data = train_data, ntree = 100, mtry = 7)

# Summary of the model
print(rf_model)

# Predict using the trained model
y_predict <- predict(rf_model, newdata = test_data)
...
regression_results(test_data$Appliances, y_predict)
...
plot_data <- data.frame(Actual = test_data$Appliances, Predicted = y_predict)
...
residuals <- test_data$Appliances - y_predict
