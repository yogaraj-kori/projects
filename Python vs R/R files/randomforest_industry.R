...
rf_model <- randomForest(Usage_kWh ~ ., data = train_data, ntree = 100, mtry = 7)
...
regression_results(test_data$Usage_kWh, y_predict)
...
plot_data <- data.frame(Actual = test_data$Usage_kWh, Predicted = y_predict)
...
residuals <- test_data$Usage_kWh - y_predict
